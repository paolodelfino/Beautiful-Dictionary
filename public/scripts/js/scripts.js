...
$(document).ready(function () {
  ...
  // if the page is loaded with word and lang variables, search the word
  if ($(word).html() && $(lang).html()) {
    $("#word-to-search").val($(word).html());
    $("#languages").val($(lang).html());
    searchWord();
  }

  let wordreference = "/api/word/page?word=&lang=";
  switch (store.get("lang")) {
    //case "Italiano":
    //  wordreference += "Italiano";
    //  break;
    case "English":
...
function searchWord() {
  ...
  if (word) {
    ...
    function handleSuccess(data) {
          ...
          ol.querySelectorAll("li").forEach((li) => {
            ...
            if (more) {
              ...
              moreDefinitions.forEach((moreDefinition) => {
                ...
                let moreExamples = moreDefinition.querySelectorAll(".rh_ex");
                if (moreExamples) {
      ...
      // and put /api/word/page and queries in front of the link href
      const tags = [
        "<strong>",
        "</strong>",
        "<b>",
      ...
      dicts.forEach((dict) => {
        ...
        if (dict.label.includes('<a href="')) {
          dict.label = dict.label.replace(
            '<a href="',
            '<a class="reference-link" href="' + wordreference.replace("word=", "word=" + word)
          );
        }
